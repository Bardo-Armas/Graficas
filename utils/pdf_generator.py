...
class PDFGenerator:
    ...
    def _try_convert_chart_to_image(self, chart_fig):
        """
        Intentar convertir gráfica de Plotly a imagen
        
        Args:
            chart_fig: Figura de Plotly
            
        Returns:
            tuple: (success: bool, image_bytes: bytes or None, error_msg: str or None)
        """
        try:
            # Intentar convertir con Kaleido
            ...
            if "Chrome" in error_msg or "kaleido" in error_msg.lower():
                return False, None, "Chrome no disponible en el servidor"
            else:
                return False, None, f"Error de conversión: {error_msg}"
    ...
    def create_pdf_with_chart_and_table(self, title, chart_fig, dataframe, filename, metrics=None):
        ...
        Returns:
            tuple: (success: bool, pdf_bytes: bytes or None, error_msg: str or None)
        """
        try:
            buffer = io.BytesIO()
            ...
            # Intentar incluir gráfica
            chart_included = False
            if chart_fig:
                success, img_bytes, error_msg = self._try_convert_chart_to_image(chart_fig)
                ...
                if success:
                    ...
                    # Agregar nota sobre la gráfica no disponible
                    story.append(Paragraph("📈 Gráfica", self.subtitle_style))
                    note_text = f"<i>Nota: La gráfica no pudo ser incluida en el PDF debido a limitaciones del servidor ({error_msg}). La gráfica está disponible en la aplicación web.</i>"
    ...
    def create_data_only_pdf(self, title, dataframe, filename, metrics=None):
        """
        Crear PDF solo con datos (sin gráfica) como fallback
        ...
        note_text = "<i>Nota: Este PDF contiene solo los datos tabulares. Las gráficas están disponibles en la aplicación web debido a limitaciones del servidor de hosting.</i>"
        story.append(Paragraph(note_text, self.styles['Normal']))
        story.append(Spacer(1, 20))
...
def create_pdf_download_button(title, chart_fig, dataframe, filename_base, metrics=None):
    """
    Crear botón de descarga de PDF con gráfica y tabla (con fallback)
    ...
    # Intentar generar PDF completo primero
    success, pdf_bytes, error_msg = pdf_generator.create_pdf_with_chart_and_table(
        title=title,
        chart_fig=chart_fig,
        dataframe=dataframe,
        filename=filename,
    ...
    if success:
        ...
        st.download_button(
            label="📄 Descargar PDF (Datos + Gráfica)",
            data=pdf_bytes,
            file_name=filename,
            mime="application/pdf",
            help="Descarga un PDF con métricas, gráfica y tabla de datos"
        )
    else:
        # Fallback: generar PDF solo con datos
        try:
            pdf_bytes = pdf_generator.create_data_only_pdf(
                title=title,
                dataframe=dataframe,
                filename=filename,
                metrics=metrics
            ...
            st.download_button(
                label="📄 Descargar PDF (Solo Datos)",
                data=pdf_bytes,
                file_name=filename,
                mime="application/pdf",
                help="Descarga un PDF con métricas y tabla de datos (gráfica disponible en la web)"
            )
            
            # Mostrar información sobre la limitación
            st.info("ℹ️ Las gráficas no están disponibles en el PDF debido a limitaciones del servidor. Puedes ver las gráficas en la aplicación web.")
            
        except Exception as e:
            st.error(f"Error al generar PDF: {str(e)}")
            st.info("💡 Descarga disponible en formato CSV como alternativa")