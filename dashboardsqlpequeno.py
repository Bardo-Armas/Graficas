...
def get_db_connection():
    """
    Función segura para conectar a la base de datos usando variables de entorno
    """
    try:
        ...
        if not all([server, database, username, password]):
            st.error("❌ Error: Faltan variables de entorno para la conexión a la base de datos")
            st.info("Asegúrate de que el archivo .env contenga: DB_SERVER, DB_DATABASE, DB_USERNAME, DB_PASSWORD")
            return None
...
def obtener_datos_pedidos_diarios(fecha_inicio, fecha_fin):
    url = "https://da-pw.mx/api/apprisa-panel/reportePedidosDiarios"
    ...
    try:
        ...
        st.error(f"Error al obtener datos de pedidos: {str(e)}")
        return None
...
def procesar_concurrencias(df, fecha):
    ...
    fig.update_layout(title=f"Concurrencia de Pedidos {fecha}", xaxis_title="Hora", yaxis_title="Pedidos Simultáneos", hovermode="x unified", showlegend=True)
...
def grafico_top_establecimientos(top_10, fecha_inicio, fecha_fin):
    fig = px.pie(top_10, names="name_restaurant", values="total_pedidos", title=f"Top 10 Establecimientos: {fecha_inicio.strftime('%Y-%m-%d')} al {fecha_fin.strftime('%Y-%m-%d')}", color_discrete_sequence=px.colors.qualitative.Pastel, hole=0.3, labels={"name_restaurant": "Establecimiento", "total_pedidos":"Pedidos"})
    return fig
...
def grafico_establecimientos_pedidos(df, fecha_inicio, fecha_fin):
    ...
    fig.update_layout(title=f"Pedidos y Establecimientos: {fecha_inicio.strftime('%Y-%m-%d')} al {fecha_fin.strftime('%Y-%m-%d')}", xaxis_title="Fecha", yaxis_title="Cantidad", hovermode="x unified")
    return fig

def grafico_pedidos_hora(contador_final, fecha):
    fig = px.bar(contador_final, x="etiqueta_hora", y="pedidos", title=f"Pedidos por hora - {fecha.strftime('%Y-%m-%d')}", labels={"etiqueta_hora": "Hora", "pedidos": "Pedidos"}, color="pedidos", color_continuous_scale="blues")
...
def procesar_datos_semanales(data_estadistica, tipo):
    df = pd.DataFrame(data_estadistica["data"]["detalle"]["general"]["todos"])
    ...
    if tipo == "pedidos":
        ...
        df_agrupado = df.groupby("fecha")[col_valor].sum().reset_index()

    df_agrupado["semana"] = calcular_semanas(df_agrupado["fecha"])
    ...
    semanal["rango_fechas"] = semanal.apply(lambda x: f"{x['fecha_inicio'].strftime('%d/%m/%Y')} - {x['fecha_fin'].strftime('%d/%m/%Y')}", axis=1)
...
def analisis_mensual():
    ...
    costo_envio = mes_actual["creditos_totales"] / mes_actual["pedidos_totales"] if mes_actual["pedidos_totales"] > 0 else 0
    ...
    if not mes_anterior.empty:
        ...
        costo_envio_ant = mes_anterior_data["creditos_totales"] / mes_anterior_data["pedidos_totales"] if mes_anterior_data["pedidos_totales"] > 0 else 0
        delta_costo = ((costo_envio - costo_envio_ant)/costo_envio_ant)*100 if costo_envio_ant > 0 else 0
        ...
        col6.metric("Costo de Envío", f"${costo_envio:.2f}", f"{delta_costo:.1f}%", delta_color="inverse" if delta_costo > 0 else "inverse")
    else:
...
def dashboard_general():
    ...
    with tab2:
        ...
        if data_estadistica:
            ...
            if not top_10.empty:
                ...
                st.download_button("Descargar datos", data=csv, file_name=f"top_establecimientos_{fecha_inicio}_{fecha_fin}.csv", mime="text/csv")
            else:
    ...
    with tab3:
        ...
        if data_estadistica:
            ...
            st.dataframe(df_establecimientospedidos[["Fecha", "Establecimientos", "Pedidos", "Promedio"]].sort_values("Fecha", ascending=True), column_config={"Fecha": st.column_config.DateColumn("Fecha"), "Establecimientos": st.column_config.NumberColumn("Establecimientos"), "Pedidos": st.column_config.NumberColumn("Pedidos"), "Promedio": st.column_config.NumberColumn("Promedio", format="%.2f")}, hide_index=True, use_container_width=True)
            csv = df_establecimientospedidos.to_csv(index=False).encode("utf-8")
            st.download_button("Descargar Datos", data=csv, file_name=f"establecimientos_pedidos_{fecha_inicio}_{fecha_fin}.csv", mime="text/csv")
        else:
    ...
    with tab4:
        ...
        if data_hora is not None:
            ...
            if not contador_horas.empty:
                ...
                st.download_button("Descargar Datos", data=csv, file_name=f"pedidos_hora_{fecha_hora}.csv", mime="text/csv")
            else:
    ...
    with tab5:
        ...
        if data_concurrencia is not None:
            ...
            if resultado[0] is not None:
                ...
                st.dataframe(df_concurrencia, column_config={"Hora": st.column_config.DatetimeColumn("Hora", format="HH:mm"), "Pedidos_Simultaneos": st.column_config.NumberColumn("Pedidos Simultáneos")}, hide_index=True, use_container_width=True)
                csv = df_concurrencia.to_csv(index=False).encode("utf-8")
                st.download_button("Descargar Datos de Concurrencia", data=csv, file_name=f"concurrencia_{fecha_concurrencia}.csv", mime="text/csv")
            else:
    ...
    with tab6:
        ...
        fecha_inicio_sem = datetime(2025, 1, 1).date()
        with st.spinner("Obteniendo datos de pedidos..."):
            ...
            if datos_estadistica_pedidos and datos_estadistica_pedidos.get("success"):
                pedidos_semanales = procesar_datos_semanales(datos_estadistica_pedidos, "pedidos")
            else:
        ...
        if pedidos_semanales is not None:
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.dataframe(pedidos_semanales[["semana", "rango_fechas", "pedidos_totales"]].rename(columns={"semana": "Semana", "rango_fechas": "Rango", "pedidos_totales": "Pedidos"}), hide_index=True)
            csv = pedidos_semanales.to_csv(index=False).encode("utf-8")
            st.download_button("Descargar Datos", data=csv, file_name=f"pedidos_semanales_{fecha_inicio_sem}_{fecha_actual}.csv", mime="text/csv")
        else:
        ...
        with st.spinner("Obteniendo datos de créditos y pedidos..."):
            ...
            if data_creditos_pedidos is not None and data_creditos_pedidos.get("success"):
                ...
                if not df_estadistica.empty:
                    creditos_semanales = procesar_datos_semanales(data_creditos_pedidos, "creditos")
                    pedidos_semanales_for_credits = procesar_datos_semanales(data_creditos_pedidos, "pedidos") 
